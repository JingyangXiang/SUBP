...
class UniformBlockConv2d(BaseNMConv):
    def __init__(self, mask_type, in_channels, out_channels, kernel_size, stride=1, **kwargs):
        super(UniformBlockConv2d, self).__init__(in_channels, out_channels, kernel_size, stride=stride, **kwargs)
    ...
    def get_mask(self, weight):
        ...
        index_prune_end_cur_epoch = int(prune_rate_cur_epoch * self.in_channels)
...
class SoftUniformBlockConv2d(UniformBlockConv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, **kwargs):
        super(SoftUniformBlockConv2d, self).__init__(SoftParameterMasker, in_channels, out_channels, kernel_size,
                                                     stride=stride, **kwargs)


class HardUniformBlockConv2d(UniformBlockConv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, **kwargs):
        super(HardUniformBlockConv2d, self).__init__(HardParameterMasker, in_channels, out_channels, kernel_size,
                                                     stride=stride, **kwargs)
